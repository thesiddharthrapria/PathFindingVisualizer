...
export default class Visualizer extends Component {
    ...
    for (let i = 1; i < shortestPathNodes.length - 1; i++) {
      ...
      setTimeout(() => {
        ...
        // else $(`#node-${node.row}-${node.col}`).removeClass("node-visited").addClass("node-path");
          ...
          if (node.col < shortestPathNodes[i-1].col)
          {
            $(`#node-${node.row}-${node.col}`).removeClass("node-visited").addClass("startleft");
          }
          else if(node.col > shortestPathNodes[i-1].col)
          {
            $(`#node-${node.row}-${node.col}`).removeClass("node-visited").addClass("startright");
          }
          else if(node.row < shortestPathNodes[i-1].row)
          {
            $(`#node-${node.row}-${node.col}`).removeClass("node-visited").addClass("startup");
          }
          else if(node.row > shortestPathNodes[i-1].row)
          {
            $(`#node-${node.row}-${node.col}`).removeClass("node-visited").addClass("startdown");
          }
  ...
  render() {
    ...
    if(topMessage.split(" ")[0] === "Dijkstra" || topMessage.split(" ")[0] === "Bellman-Ford" ) {
      ...
      <div className="visualizerBox">
        <div className="container shadow-lg p-4 mb-5 rounded">
          ...
          <div class="startButtonSection">
            <button type="button" class="btn btn-info startButton">{mainButton}</button>
          </div>
